...
public class UserRightsActivity extends AppCompatActivity {
    ...
    private void intiData() {
        ...
        rightsFragments.add(RightsFragment.newInstance(allocatedRightList, true));  //已分配权限
        rightsFragments.add(RightsFragment.newInstance(unallocatedRightList, false));  //未分配权限
    ...
    private void initRightList() {
        //查询用户未分配的所有权限
        Cursor cursor = mySQLiteOpenHelper.getReadableDatabase().rawQuery("select rights.right_NO,rights.right_name,rights.right_module,rights.win_name from users,rights " +
        ...
        while (cursor != null && cursor.moveToNext()) {
            Right right = new Right(cursor.getInt(0), cursor.getString(1), cursor.getString(2), cursor.getString(3));
            unallocatedRightList.add(right);
        }
        if (cursor != null)
            cursor.close();
        //查询用户已分配的所有权限
        Cursor cursor2 = mySQLiteOpenHelper.getReadableDatabase().rawQuery("select rights.right_NO,rights.right_name,rights.right_module,rights.win_name from users,rights " +
        ...
        while (cursor2 != null && cursor2.moveToNext()) {
            Right right = new Right(cursor2.getInt(0), cursor2.getString(1), cursor2.getString(2), cursor2.getString(3));
        ...
        while (cursor3 != null && cursor3.moveToNext()) {
            Log.i(TAG, "initRightList: " + cursor3.getInt(1) + cursor3.getString(2) + cursor3.getString(3) + cursor3.getString(4));
        }
    }
